...
def cadastro(request):
    ...
    if form.is_valid():
        ...
        return render(request, 'cadastro.html', {'form': form})  # Exibir erros de validação
...
def editar_perfil(request):
    if request.method == 'POST':
        ...
        # Verificar se o perfil existe, caso contrário, criar um novo
        if not hasattr(request.user, 'profile'):
            request.user.profile = Profile.objects.create(user=request.user)

        request.user.profile.biografia = request.POST.get('biografia')  # Atualizar biografia
        request.user.save()
        request.user.profile.save()  # Salvar o perfil
